...
def welcome(pm):
    ...
    bot.register_next_step_handler(sent_msg, first_handler) #Next message will call the name_handler function
    
def first_handler(pm):
    first = pm.text
    sent_msg = bot.send_message(pm.chat.id, f"First accused is {first}. Send in the name of the second accused.")
    bot.register_next_step_handler(sent_msg, second_handler, first) #Next message will call the age_handler function
